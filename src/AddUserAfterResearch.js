...
class AddUserAfterResearch extends Component {
    ...
    componentDidMount() {
        ...
        fetch('  https://modamedic.cs.bgu.ac.il/users/getVerifications')
            .then(response => {
        ...
        fetch('  https://modamedic.cs.bgu.ac.il/questionnaires/all')
            .then(response => {
                return response.json();
            }).then(results => {
    ...
    handleSubmit(event) {
        ...
        if(this.state.type === 'doctor') {
            axios.post('  https://modamedic.cs.bgu.ac.il/users/doctorRegister', {
        ...
        if(this.state.type === 'patient') {
            ...
            axios.post('  https://modamedic.cs.bgu.ac.il/users/patientRegister', {
                UserID: this.state.userName,
                Password: this.state.password,
                First_Name: this.state.fName,
                Last_Name: this.state.lName,
