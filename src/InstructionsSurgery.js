...
class InstructionsSurgery extends Component {
    ...
    async getInstructions() {
        let respone = await axios.get('  https://modamedic.cs.bgu.ac.il/auth/usersAll/instructions',
            {
                headers: {
    ...
    downloadFile = (instruction) => {
        axios({
            url: ` https://modamedic.cs.bgu.ac.il/auth/usersAll/instructions/${instruction.InstructionId}`,
            method: 'GET',
    ...
    uploadFile(category, title, file, onUploadProgress) {
        ...
        return axios.post(" https://modamedic.cs.bgu.ac.il/auth/doctors/instructions", formData, {
            headers: {
    ...
    async removeInstruction(eId){
        if(sessionStorage.getItem('doctor')) {
            ...
            if (r) {
                await axios.delete(`  https://modamedic.cs.bgu.ac.il/auth/doctors/instructions/${eId}`,
                    {
                        headers: {
                            'Content-Type': 'application/json',
                            'x-auth-token': sessionStorage.getItem("token")
